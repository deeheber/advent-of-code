...
try {
  ...
  for (let i = 0; i < rucksacks.length; i += 3) {
    ...
    const firstSecondIntersect = new Set(
      [...firstSet].filter((x) => secondSet.has(x))
    )
    const secondThirdIntersect = new Set(
      [...secondSet].filter((x) => thirdSet.has(x))
    )
    const finalIntersect = new Set(
      [...firstSecondIntersect].filter((x) => secondThirdIntersect.has(x))
    )
